...
category: telemetry
trigger: "Operator wants to submit anonymized telemetry to the Spacemacs.aDNA upstream channel — typically after a skill_self_improve cycle landed an ADR worth sharing with the fleet."
phase_introduced: 8
...
requirements:
  ...
  permissions:
    ...
    - "gh issue create against LatticeProtocol/Spacemacs.aDNA"
---
...
Operator-gated wrapper around `gh issue create` that submits anonymized telemetry to the upstream `LatticeProtocol/Spacemacs.aDNA` repo (label: `telemetry`).
...
This payload will be submitted as a GitHub issue to:
  LatticeProtocol/Spacemacs.aDNA (label: telemetry)
...
BODY=$(cat <<'JSON'
<!-- Spacemacs.aDNA telemetry submission -->
...
```bash
gh api repos/LatticeProtocol/Spacemacs.aDNA/issues/<N>/comments \
